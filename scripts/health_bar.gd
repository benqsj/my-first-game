...
##
## Two flat quads — a dark backing and a coloured fill — both set to billboard,
## so they face the camera from any angle without anything driving them. The
## fill is scaled from its left edge rather than its centre, which is why it is
## parented to an offset pivot: scaling a quad about its middle would shrink it
## inwards from both ends instead of draining it.
